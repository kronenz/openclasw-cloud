...
export function Sidebar() {
  ...
  const displayName = isAdmin ? 'Operator' : (payload.sub?.replace('tn_', '').slice(0, 8) || 'User')
    ...
    <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="px-3 py-4 border-t border-gray-200">
        <div className="flex items-center gap-3 px-3">
          <div className={`w-8 h-8 rounded-full ${isAdmin ? 'bg-blue-100' : 'bg-green-100'} flex items-center justify-center`}>
            <Users className={`w-4 h-4 ${isAdmin ? 'text-blue-600' : 'text-green-600'}`} />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-800 truncate">{displayName}</p>
            <p className="text-xs text-gray-500 truncate">{isAdmin ? 'Admin' : 'Tenant'}</p>
          </div>
